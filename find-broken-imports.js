  ...
  for (const file of files) {
    ...
    try {
      ...
      for (const importPath of imports) {
        ...
        if (importPath.match(/^[a-z]/i) && !importPath.startsWith('@/') && !importPath.startsWith('.')) {
          continue;
        }
  ...
  if (brokenImports.length === 0) {
    ...
    for (const file of sortedFiles) {
      ...
      for (const broken of byFile[file]) {
        const importStr = `import X from '${broken.importPath}'`;
        if (!uniqueImports.has(importStr)) {
          uniqueImports.add(importStr);
          console.log(`   ❌ import X from '${broken.importPath}'`);
